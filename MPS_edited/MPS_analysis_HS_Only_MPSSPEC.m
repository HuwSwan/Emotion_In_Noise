function [MS, TF, roughness, cycleSum, tempSum, xAxis, mpsIdx, mpsBands] = MPS_analysis_HS_Only_MPSSPEC(wavefile,fs)
%% Huw Swanborough Edit to output the TF structure.

%% Only prints the MPS Spectra, nothing else.
...
imDat = imgaussfilt(MS.val,1);

imagesc(MS.x,MS.y,imDat,[C B]); axis xy
axe = gca();
