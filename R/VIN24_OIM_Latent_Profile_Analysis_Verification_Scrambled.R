...
pfile <- 'X:/PhD/03-Original_OIM/02-analysis/2024-verification-dwgp_collation/scram/window_features_speech/summary_windows.csv'
...
p1 <- ggplot(data = fits) +
  ...
  theme_minimal() + xlab("K models of 1:6 classes") +
...
ggsave("AIC_scrambled.svg",plot = p1,units = "cm", dpi = 500,width = 5,height = 5)

##class solution push through

fitdat <- data.frame(c4$model_1_class_4$dff)

fitdat$ID <- row.names(datx)
## Degreees of freedom and membership probs.
probs <-data.frame(c4$model_1_class_4$dff)
...
#datx$CPROB4 <- as.factor(probs$CPROB5)
#datx$CPROB4 <- as.factor(probs$CPROB6)




saveStr <- 'X:/PhD/03-Original_OIM/02-analysis/2024-verification-dwgp_collation/scram/window_features_speech/window_data_classes_4class.csv'
